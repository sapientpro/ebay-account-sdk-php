 ...
 */
class FulfillmentPolicy implements EbayModelInterface
{
    use FillsModel;
    ...
    /**
     * If returned as <code>true</code>,
     * eBay's Global Shipping Program will be used by the seller to ship items to international locations.
     * <span class="tablenote">Note: On the US marketplace, the <em>Global Shipping Program</em>
     * is scheduled to be replaced by a new intermediated international shipping program called
     * <em>eBay International Shipping</em>.
     * US sellers who are opted in to the Global Shipping Program
     * will be automatically opted in to eBay International Shipping when it becomes available to them.
     * All US sellers will be migrated by March 31, 2023.
     * eBay International Shipping is an account level setting,
     * and no field needs to be set in a Fulfillment business policy to enable it.
     * As long as the US seller's account is opted in to eBay International Shipping,
     * this shipping option will be enabled automatically for all listings where international shipping is available.
    ...
    /**
     * Specifies the maximum number of business days the seller commits to
     * for preparing and shipping an order after receiving a cleared payment for the order.
     * This time does not include the transit time it takes the shipping carrier to deliver the order.
     * <p>If only local pickup or freight shipping is available for the item, this container may not get returned.</p>
     *
     ...
     * A separate ShippingOption object covers domestic shipping service options and international shipping service options (if the seller ships to international locations).
     ...
     * which define the geographical regions/countries/states or provinces/domestic regions where
     * the seller does and doesn't ship to with this fulfillment policy.
     *
     * @var RegionSet|null
     */
